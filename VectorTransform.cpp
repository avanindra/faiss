...
void LinearTransform::apply_noalloc (int64_t n, const float * x,
                               float * xt) const
{
...
void eig(size_t d_in, double *cov, double *eigenvalues, int verbose)
{
    { // compute eigenvalues and vectors
        ...
        if(verbose && d_in <= 10) {
            printf("info=%ld new eigvals=[",  int64_t(info));
...
void PCAMatrix::train (int64_t n, const float *x)
{
    const float * x_in = x;
...
void OPQMatrix::train (int64_t n, const float *x)
{
