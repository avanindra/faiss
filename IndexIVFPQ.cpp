...
struct InvertedListScanner: QueryTables {
    ...
    void init_list (int64_t key, float coarse_dis,
                                     ...
                                     ) const
{
    ...
    int64_t local_nprobe = params ? params->nprobe : nprobe;
    int64_t local_max_codes = params ? params->max_codes : max_codes;

#pragma omp parallel
    {
        InvertedListScanner<int64_t> qt (*this, params);
...
#pragma omp  for
        for (size_t i = 0; i < nx; i++) {
            ...
            const int64_t * keysi = keys + i * local_nprobe;
                                      ...
                                      ) const
{
    uint64_t t0;
    TIC;
    size_t k_coarse = int64_t(k * k_factor);
