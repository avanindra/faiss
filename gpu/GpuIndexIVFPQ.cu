...
void
GpuIndexIVFPQ::trainResidualQuantizer_(int64_t n, const float* x) {
  // Code largely copied from faiss::IndexIVFPQ
  // FIXME: GPUize more of this
  n = std::min(n, (int64_t) (1 << bitsPerCode_) * 64);
...
void
GpuIndexIVFPQ::train(int64_t n, const float* x) {
  DeviceScope scope(device_);
...
void
GpuIndexIVFPQ::addImpl_(int64_t n,
                        const float* x,
                        const int64_t* xids) {
...
void
GpuIndexIVFPQ::searchImpl_(int64_t n,
                           const float* x,
                           int64_t k,
                           float* distances,
