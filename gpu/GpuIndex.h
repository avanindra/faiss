...
class GpuIndex : public faiss::Index {
 public:
  ...
  void add(int64_t, const float* x) override;

  /// `x` and `ids` can be resident on the CPU or any GPU; copies are
  /// performed as needed
  /// Handles paged adds if the add set is too large; calls addInternal_
  void add_with_ids(int64_t n, const float* x, const int64_t* ids) override;
 ...
 protected:
  ...
  void addInternal_(int64_t n,
                    const float* x,
                    const int64_t* ids);

  /// Overridden to actually perform the add
  virtual void addImpl_(int64_t n,
                        const float* x,
                        const int64_t* ids) = 0;

  /// Overridden to actually perform the search
  virtual void searchImpl_(int64_t n,
                           const float* x,
                           int64_t k,
                           float* distances,
