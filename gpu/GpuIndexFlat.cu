...
void
GpuIndexFlat::train(int64_t n, const float* x) {
  // nothing to do
}

void
GpuIndexFlat::add(int64_t n, const float* x) {
  DeviceScope scope(device_);
...
void
GpuIndexFlat::addImpl_(int64_t n,
                       const float* x,
                       const int64_t* ids) {
...
struct IntToint64_t {
  __device__ int64_t operator()(int v) const { return (int64_t) v; }
};

void
GpuIndexFlat::search(int64_t n,
                     const float* x,
                     int64_t k,
                     float* distances,
  ...
  auto outIndices = toDevice<int64_t, 2>(resources_,
                                                     device_,
                                                     labels,
                                                     stream,
                                                     {(int) n, (int) k});
  ...
  fromDevice<int64_t, 2>(outIndices, labels, stream);
}

void
GpuIndexFlat::searchImpl_(int64_t n,
                          const float* x,
                          int64_t k,
                          float* distances,
                          int64_t* labels) const {
...
void
GpuIndexFlat::reconstruct(int64_t key,
                          float* out) const {
  DeviceScope scope(device_);
...
void
GpuIndexFlat::reconstruct_n(int64_t i0,
                            int64_t num,
                            float* out) const {
  DeviceScope scope(device_);
