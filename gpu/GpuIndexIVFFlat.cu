...
void
GpuIndexIVFFlat::train(int64_t n, const float* x) {
  DeviceScope scope(device_);
...
void
GpuIndexIVFFlat::addImpl_(int64_t n,
                          const float* x,
                          const int64_t* xids) {
  ...
  //static_assert(sizeof( int64_t) == sizeof(int64_t), "size mismatch"); TODO
  auto deviceIds =
...
void
GpuIndexIVFFlat::searchImpl_(int64_t n,
                             const float* x,
                             int64_t k,
                             float* distances,
  ...
  fromDevice<int64_t, 2>(devLabels, labels, stream);
}
