...
void ParameterSpace::initialize (const Index * index)
{
    ...
    if (DC (IndexIVF)) {
        ...
        if (miq) {
			...
			//FIX : avanindra
			pr_max_codes.values.push_back(std::numeric_limits<double>::infinity());//(1.0 / 0.0);
        }
    }
    if (DC (IndexIVFPQR)) {
     ...
     Index *index, const char *description_in) const
{
    ...
    for (char *tok = strtok_s (description, " ,", &ptr);
         tok;
         tok = strtok_s (nullptr, " ,", &ptr)) {
        char name[100];
        double val;
                              ...
                              OperatingPoints * ops) const
{
    ...
    if (n_experiments == 0) {
        ...
        for (size_t cno = 0; cno < n_comb; cno++) {
            ...
            std::vector<int64_t> I(nq * crit.nnn);
