...
#pragma omp parallel for reduction(+: nlistv, ndis)
    for (size_t i = 0; i < nx; i++) {
        ...
        for (size_t ik = 0; ik < nprobe; ik++) {
            ...
            const int64_t * ids = store_pairs ? nullptr :
                                       ...
                                       const IVFSearchParameters *params) const
{
   if (metric_type == METRIC_INNER_PRODUCT) {
        ...
        if (store_pairs) {
            search_knn_for_ivf<METRIC_INNER_PRODUCT, true, CMin<float,  int64_t> >
                (*this, n, x, idx, &res, params);
        } else {
            search_knn_for_ivf<METRIC_INNER_PRODUCT, false, CMin<float,  int64_t> >
                (*this, n, x, idx, &res, params);
        }
        ...
        float_maxheap_array_t res = {
            size_t(n), size_t(k), labels, distances};
        if (store_pairs) {
            search_knn_for_ivf<METRIC_L2, true, CMax<float,  int64_t> >
                (*this, n, x, idx, &res, params);
        } else {
            search_knn_for_ivf<METRIC_L2, false, CMax<float,  int64_t> >
                (*this, n, x, idx, &res, params);
        }
    }
}
...
#pragma omp parallel
    {
        ...
        for (size_t i = 0; i < nx; i++) {
            ...
            for (size_t ik = 0; ik < nprobe; ik++) {
                ...
                const int64_t * ids = invlists->get_ids (key);
...
static uint64_t hash_bytes (const uint8_t *bytes,  int64_t n) {
