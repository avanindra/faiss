...
void IndexBinaryIVF::add_with_ids(idx_t n, const uint8_t *x, const int64_t *xids) {
  add_core(n, x, xids, nullptr);
}

void IndexBinaryIVF::add_core(idx_t n, const uint8_t *x, const int64_t *xids,
  ...
  std::unique_ptr<int64_t[]> scoped_idx;

  if (precomputed_idx) {
    idx = precomputed_idx;
  } else {
    scoped_idx.reset(new int64_t[n]);
...
void IndexBinaryIVF::reconstruct(idx_t key, uint8_t *recons) const {
  ...
  int64_t list_no = direct_map[key] >> 32;
  int64_t offset = direct_map[key] & 0xffffffff;
...
void IndexBinaryIVF::search_and_reconstruct(idx_t n, const uint8_t *x, idx_t k,
  ...
  std::unique_ptr<idx_t[]> idx(new int64_t[n * nprobe]);
  std::unique_ptr<int32_t[]> coarse_dis(new int32_t[n * nprobe]);
...
void IndexBinaryIVF::reconstruct_from_offset(int64_t list_no, int64_t offset,
                                             uint8_t *recons) const {
...
void search_knn_hamming_count(const IndexBinaryIVF& ivf,
  ...
  std::unique_ptr<int64_t[]> all_ids_per_dis(new int64_t[nx * nBuckets * k]);
