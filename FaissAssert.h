...
#define FAISS_ASSERT_FMT(X, FMT, ...)                                   \
  do {                                                                  \
    if (! (X)) {                                                        \
      fprintf(stderr, "Faiss assertion '%s' failed in %s "              \
               "at %s:%d; details: " FMT "\n",                          \
               #X, __FUNCTION__, __FILE__, __LINE__, __VA_ARGS__); \
      abort();                                                          \
    }                                                                   \
  } while (false)
...
#define FAISS_THROW_MSG(MSG)                                            \
  do {                                                                  \
    throw faiss::FaissException(MSG, __FUNCTION__, __FILE__, __LINE__); \
  } while (false)
...
#define FAISS_THROW_FMT(FMT, ...)                                       \
  do {                                                                  \
    ...
    throw faiss::FaissException(__s, __FUNCTION__, __FILE__, __LINE__); \
  } while (false)
