...
void ReconstructFromNeighbors::reconstruct(storage_idx_t i, float *x, float *tmp) const
{
    ...
    if (k == 1 || nsq == 1) {
        ...
        //FIX
		float const  **betas = new float const*[nsq];//const 
        {
...
struct DistanceXPQ4: Distance2Level {
    ...
    float operator () (storage_idx_t i) override
    {
        ...
        for (int m = 0; m < M; m++) {
			...
			__m128 recons = _mm_add_ps(l1_t[m], pq_l2_t[*code++]);  //l1_t[m] + pq_l2_t[*code++];
			__m128 diff = _mm_sub_ps(qi, recons); //qi - recons;
			accu = _mm_add_ps(accu , _mm_mul_ps(diff, diff)); //diff * diff;
            pq_l2_t += 256;
            qa += 4;
        }
...
struct Distance2xXPQ4: Distance2Level {
    ...
    float operator () (storage_idx_t i) override
    {
        ...
        for (int mi_m = 0; mi_m < 2; mi_m++) {
            ...
            for (int m = 0; m < M_2; m++) {
                __m128 qi = _mm_loadu_ps(qa);
				__m128 recons = _mm_add_ps(pq_l1[m], pq_l2_t[*code++]);//pq_l1[m] + pq_l2_t[*code++];
				__m128 diff = _mm_sub_ps(qi, recons); //qi - recons;
				accu = _mm_add_ps(accu, _mm_mul_ps(diff, diff)); //accu += diff * diff;
                pq_l2_t += 256;
                qa += 4;
            }
